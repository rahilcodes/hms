...
class StaffController extends Controller
{
    ...
    /**
     * Granular permission toggles (only meaningful for non-manager roles —
     * super_admin/admin always pass hasPermission()).
     */
    protected function permissionsFromRequest(Request $request): array
    {
        $toggles = $request->input('permissions', []);

        return [
            'can_discount' => in_array('can_discount', $toggles),
            'can_void' => in_array('can_void', $toggles),
            'can_override_rate' => in_array('can_override_rate', $toggles),
            'max_discount_percent' => (float) $request->input('max_discount_percent', 10),
        ];
    }
