...
class GuestRegistrationController extends Controller
{
    ...
    public function update(Request $request, $uuid)
    {
        ...
        $validated = $request->validate([
            ...
            'id_type' => 'nullable|string|in:aadhaar,passport,driving_license,voter_id,pan',
            'id_number' => 'nullable|string|max:50',
            'visa_number' => 'nullable|string|max:50',
            'visa_expiry' => 'nullable|date',
            'arrived_from' => 'nullable|string|max:150',
            'next_destination' => 'nullable|string|max:150',
        ]);
        ...
        $guest->update([
            ...
            'id_type' => $validated['id_type'] ?? $guest->id_type,
            'id_number' => $validated['id_number'] ?? $guest->id_number,
            'visa_number' => $validated['visa_number'] ?? $guest->visa_number,
            'visa_expiry' => $validated['visa_expiry'] ?? $guest->visa_expiry,
            'arrived_from' => $validated['arrived_from'] ?? $guest->arrived_from,
            'next_destination' => $validated['next_destination'] ?? $guest->next_destination,
        ]);
