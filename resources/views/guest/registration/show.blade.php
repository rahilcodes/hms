        ...
        <div class="max-w-3xl mx-auto">
            ...
            <form action="{{ route('guest.registration.update', $booking->uuid) }}" method="POST"
                ...
                <div x-show="step === 1" x-transition:enter="transition ease-out duration-300 transform"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-xs font-bold text-slate-500 uppercase mb-1">ID Type</label>
                            <select name="id_type"
                                class="w-full border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 font-medium text-slate-900">
                                <option value="">— Select —</option>
                                <option value="aadhaar" @selected(old('id_type', $guest->id_type) === 'aadhaar')>Aadhaar Card</option>
                                <option value="passport" @selected(old('id_type', $guest->id_type) === 'passport')>Passport</option>
                                <option value="driving_license" @selected(old('id_type', $guest->id_type) === 'driving_license')>Driving License</option>
                                <option value="voter_id" @selected(old('id_type', $guest->id_type) === 'voter_id')>Voter ID</option>
                                <option value="pan" @selected(old('id_type', $guest->id_type) === 'pan')>PAN Card</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-500 uppercase mb-1">ID Number</label>
                            <input type="text" name="id_number" value="{{ old('id_number', $guest->id_number) }}"
                                class="w-full border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 font-medium text-slate-900 placeholder:text-slate-300"
                                placeholder="As on the document">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Arrived From</label>
                            <input type="text" name="arrived_from" value="{{ old('arrived_from', $guest->arrived_from) }}"
                                class="w-full border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 font-medium text-slate-900 placeholder:text-slate-300"
                                placeholder="City / Country">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Next Destination</label>
                            <input type="text" name="next_destination" value="{{ old('next_destination', $guest->next_destination) }}"
                                class="w-full border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 font-medium text-slate-900 placeholder:text-slate-300"
                                placeholder="City / Country">
                        </div>
                    </div>

                    {{-- Foreign nationals: visa details for the C-Form / FRRO report --}}
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 rounded-2xl bg-blue-50/60 border border-blue-100">
                        <p class="md:col-span-2 text-xs font-bold text-blue-700">Foreign nationals only — required for the Government of India C-Form</p>
                        <div>
                            <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Visa Number</label>
                            <input type="text" name="visa_number" value="{{ old('visa_number', $guest->visa_number) }}"
                                class="w-full border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 font-medium text-slate-900">
                        </div>
                        <div>
                            <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Visa Valid Till</label>
                            <input type="date" name="visa_expiry" value="{{ old('visa_expiry', optional($guest->visa_expiry)->toDateString()) }}"
                                class="w-full border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 font-medium text-slate-900">
                        </div>
                    </div>
