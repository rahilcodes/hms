...
<div class="min-h-screen bg-slate-50 pb-20">
    ...
    <div class="max-w-7xl mx-auto px-6 mt-12 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-8 space-y-8">
            ...
            {{-- UPI SCAN & PAY --}}
            @php $upiLink = $booking->balance_amount > 0 ? upi_uri((float) $booking->balance_amount, 'Booking #' . ($booking->id + 1000)) : null; @endphp
            @if($upiLink)
                <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 flex flex-col sm:flex-row items-center gap-6">
                    <div data-upi-qr="{{ $upiLink }}" data-qr-size="150" class="p-2.5 bg-white border-2 border-slate-100 rounded-2xl shrink-0"></div>
                    <div class="text-center sm:text-left">
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Scan &amp; Pay via UPI</p>
                        <p class="text-lg font-black text-slate-900 mt-1">Clear your balance of ₹{{ number_format($booking->balance_amount) }} instantly</p>
                        <p class="text-xs text-slate-500 mt-1.5">Pay to <span class="font-bold">{{ site('hotel_upi_vpa') }}</span> from GPay, PhonePe or Paytm — then show the confirmation at the desk.</p>
                        <a href="{{ $upiLink }}" class="mt-3 inline-block px-5 py-2.5 rounded-xl bg-emerald-600 text-white text-xs font-black sm:hidden">Open UPI App &rarr;</a>
                    </div>
                </div>
            @endif
            ...
            <div class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden p-10">
                ...
                {{-- MULTI-ROOM TRIP SUMMARY --}}
                @if(count($groupBookings) > 0)
                    <div class="mt-8 pt-8 border-t border-slate-100">
                        <h5 class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6">Your Multi-room Trip</h5>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            @foreach($groupBookings as $gb)
                                <div class="p-6 bg-slate-50 rounded-3xl border border-slate-100">
                                    <div class="flex justify-between items-start mb-4">
                                        <div>
                                            <h4 class="font-black text-slate-800 text-sm">{{ $gb->roomType->name }}</h4>
                                            <p class="text-[9px] text-slate-500 font-bold uppercase tracking-widest">
                                                @if($gb->assignedRooms->count() > 0)
                                                    Room {{ $gb->assignedRooms->first()->room_number }}
                                                @else
                                                    Reservation #{{ $gb->id + 1000 }}
                                                @endif
                                            </p>
                                        </div>
                                        <a href="{{ route('guest.switch', $gb) }}" class="text-[9px] font-black uppercase text-blue-600 hover:text-blue-800 transition">Manage →</a>
                                    </div>
                                    
                                    @if(!empty($gb->services_json))
                                        <div class="flex flex-wrap gap-2">
                                            @foreach($gb->services_json as $s)
                                                <span class="px-2 py-1 bg-white border border-slate-200 rounded-lg text-[9px] font-bold text-slate-600 flex items-center gap-1 uppercase tracking-tighter">
                                                    <svg class="w-2.5 h-2.5 text-emerald-500 font-black" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="4" d="M5 13l4 4L19 7"></path></svg>
                                                    {{ $s['name'] }}
                                                </span>
                                            @endforeach
                                        </div>
                                    @else
                                        <p class="text-[9px] text-slate-400 font-black uppercase tracking-widest italic opacity-60">No Add-ons</p>
                                    @endif
                                </div>
                            @endforeach
                        </div>
