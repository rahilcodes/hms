    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 md:p-8">
            ...
            >
                ...
                {{-- INDIA PRICING: children / mattress / day-use --}}
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Child Price (5–12 yrs, ₹/night)</label>
                        <input type="number" name="child_price" value="{{ $roomType->child_price ?? 0 }}" min="0" step="0.01"
                            class="w-full p-2.5 bg-gray-50 border border-gray-300 rounded focus:ring-blue-500 focus:border-blue-500">
                        <p class="text-xs text-gray-500 mt-1">Below 5 stay free. 5–12 charged this rate.</p>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1">Extra Mattress (₹/night)</label>
                        <input type="number" name="extra_mattress_price" value="{{ $roomType->extra_mattress_price ?? 0 }}" min="0" step="0.01"
                            class="w-full p-2.5 bg-gray-50 border border-gray-300 rounded focus:ring-blue-500 focus:border-blue-500">
                    </div>
                </div>
                <div class="p-4 bg-blue-50/50 border border-blue-100 rounded-lg space-y-3">
                    <label class="flex items-center gap-2 text-sm font-semibold text-gray-700">
                        <input type="checkbox" name="day_use_enabled" value="1" @checked($roomType->day_use_enabled) class="w-4 h-4 rounded">
                        Enable Day-Use (hourly) booking
                    </label>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-semibold text-gray-600 mb-1">4-hour slab (₹)</label>
                            <input type="number" name="day_use_price_4h" value="{{ $roomType->day_use_price_4h }}" min="0" step="0.01"
                                class="w-full p-2.5 bg-white border border-gray-300 rounded">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-gray-600 mb-1">8-hour slab (₹)</label>
                            <input type="number" name="day_use_price_8h" value="{{ $roomType->day_use_price_8h }}" min="0" step="0.01"
                                class="w-full p-2.5 bg-white border border-gray-300 rounded">
                        </div>
                    </div>
                </div>
