    ...
    <div class="space-y-6">
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
            <div class="flex items-center justify-between mb-5">
                <div>
                    <h3 class="font-bold text-lg text-slate-900">Festival Calendar — Next 120 Days</h3>
                    <p class="text-xs text-slate-500 mt-0.5">One tap creates festival-window season rates for every room type.</p>
                </div>
            </div>
            <div class="grid sm:grid-cols-2 xl:grid-cols-3 gap-3">
                @forelse($festivals as $festival)
                    <div class="rounded-2xl border {{ $festival->rule_exists ? 'border-emerald-200 bg-emerald-50/40' : 'border-slate-200' }} p-4">
                        <div class="flex items-start justify-between gap-2">
                            <div>
                                <p class="font-black text-slate-800 text-sm">{{ $festival->name }}</p>
                                <p class="text-[11px] text-slate-500 mt-0.5">
                                    {{ $festival->date->format('d M') }}@if($festival->end_date && !$festival->end_date->isSameDay($festival->date)) – {{ $festival->end_date->format('d M Y') }} @else {{ $festival->date->format('Y') }} @endif
                                    &bull; {{ $festival->date->diffForHumans() }}
                                </p>
                            </div>
                            <span class="text-[10px] font-black px-2 py-1 rounded-full {{ $festival->occupancy >= 60 ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500' }}">{{ $festival->occupancy }}% booked</span>
                        </div>
                        <form method="POST" action="{{ route('admin.yield.festival', $festival) }}" class="mt-3 flex items-center gap-2">
                            @csrf
                            <div class="flex items-center rounded-xl border border-slate-200 bg-white overflow-hidden">
                                <span class="pl-2.5 text-xs font-black text-slate-400">+</span>
                                <input type="number" name="uplift" value="{{ $festival->suggested_uplift_percent }}" min="0" max="200"
                                    class="w-14 px-1 py-2 text-sm font-bold text-center border-0 focus:ring-0">
                                <span class="pr-2.5 text-xs font-black text-slate-400">%</span>
                            </div>
                            @if($festival->rule_exists)
                                <button class="flex-1 py-2 rounded-xl bg-emerald-600 text-white text-[11px] font-black">Update Rates</button>
                            @else
                                <button class="flex-1 py-2 rounded-xl bg-slate-900 text-white text-[11px] font-black hover:bg-blue-600 transition">Apply Surge</button>
                            @endif
                        </form>
                    </div>
                @empty
                    <p class="text-sm text-slate-400 sm:col-span-2 xl:col-span-3">No festivals seeded for the next 120 days.</p>
                @endforelse
            </div>
        </div>
