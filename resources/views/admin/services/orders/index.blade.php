...
@section('header_title', 'Service & Amenity Requests')

@section('content')
    <div class="space-y-6">
        <div class="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
            <div class="p-8 border-b border-slate-50 flex justify-between items-center">
                <h3 class="font-black text-slate-900 text-lg">Active Requests</h3>
                <span
                    ...
                    <thead>
                        <tr class="bg-slate-50/50">
                            ...
                            <th class="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Guest /
                                Room</th>
                            <th class="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Service Requested</th>
                            <th class="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Cost</th>
                        ...
                        @foreach($orders as $order)
                            <tr class="hover:bg-slate-50/50 transition">
                                    ...
                                    <div class="space-y-1">
                                        @foreach($order->items as $item)
                                            <div class="flex items-center gap-2">
                                                <span class="w-6 h-6 rounded-full bg-blue-50 flex items-center justify-center text-[10px] font-black text-blue-600">
                                                    {{ $item['qty'] }}
                                                </span>
                                ...
                                <td class="px-8 py-6">
                                    ...
                                    @else
                                        <span class="px-2 py-1 bg-slate-100 text-slate-500 rounded text-[10px] font-bold uppercase tracking-wider">Complimentary</span>
                                    @endif
                                </td>
                                <td class="px-8 py-6">
                                ...
                                <td class="px-8 py-6">
                                    <form action="{{ route('admin.services.orders.updateStatus', $order) }}" method="POST"
