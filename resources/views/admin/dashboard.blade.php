...
@section('content')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-12">
        ...
        {{-- REPUTATION --}}
        <div class="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm p-8 lg:col-span-2">
            @php
                $reviewLink = trim((string) site('google_review_link', ''));
                $reviewsAsked = \App\Models\Booking::where('status', 'checked_out')
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h3 class="text-lg font-black text-slate-900 tracking-tight">Reputation</h3>
                    <p class="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-0.5">Google reviews drive walk-ins</p>
                    <p class="text-sm text-slate-500 mt-3">
                        @if($reviewLink !== '')
                            <span class="font-black text-slate-800">{{ $reviewsAsked }}</span> guests checked out this month — each got a WhatsApp review nudge automatically.
                        @else
                            Add your <span class="font-bold">Google Review Link</span> in Site Settings and every checked-out guest gets an automatic WhatsApp review request.
                        @endif
                    </p>
                </div>
                <div class="flex gap-2 shrink-0">
                    @if($reviewLink !== '')
                        <a href="{{ $reviewLink }}" target="_blank" class="px-4 py-2.5 rounded-xl bg-slate-900 text-white text-xs font-black">View Google Reviews</a>
                    @else
                        <a href="{{ route('admin.site-settings.index') }}" class="px-4 py-2.5 rounded-xl bg-blue-600 text-white text-xs font-black">Set Review Link</a>
                    @endif
                </div>
            </div>
        </div>
