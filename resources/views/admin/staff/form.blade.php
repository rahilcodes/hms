    ...
    <form action="{{ isset($staff) ? route('admin.staff.update', $staff) : route('admin.staff.store') }}" method="POST" class="space-y-6">
        ...
        <div class="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-10 space-y-10">
            ...
            <div class="pt-10 border-t border-slate-100">
                <div class="mb-6">
                    <h4 class="text-sm font-bold text-slate-900">Front-Desk Powers</h4>
                    <p class="text-xs text-slate-400 font-medium">Applies to receptionist / revenue / housekeeping roles. Admins and Super Admins always have all powers.</p>
                </div>
                @php $perms = old('permissions', $staff->permissions ?? []); @endphp
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <label class="flex items-start gap-3 p-4 bg-slate-50 border border-slate-100 rounded-2xl cursor-pointer">
                        <input type="checkbox" name="permissions[]" value="can_discount" class="mt-0.5 w-4 h-4 rounded"
                            @checked(data_get($perms, 'can_discount', in_array('can_discount', is_array($perms) ? $perms : [])))>
                        <span>
                            <span class="block text-sm font-bold text-slate-800">Give discounts</span>
                            <span class="block text-[10px] text-slate-400 mt-0.5">Apply coupons / reduce bills up to the limit below</span>
                        </span>
                    </label>
                    <label class="flex items-start gap-3 p-4 bg-slate-50 border border-slate-100 rounded-2xl cursor-pointer">
                        <input type="checkbox" name="permissions[]" value="can_void" class="mt-0.5 w-4 h-4 rounded"
                            @checked(data_get($perms, 'can_void', in_array('can_void', is_array($perms) ? $perms : [])))>
                        <span>
                            <span class="block text-sm font-bold text-slate-800">Cancel / void</span>
                            <span class="block text-[10px] text-slate-400 mt-0.5">Cancel bookings and void bills</span>
                        </span>
                    </label>
                    <label class="flex items-start gap-3 p-4 bg-slate-50 border border-slate-100 rounded-2xl cursor-pointer">
                        <input type="checkbox" name="permissions[]" value="can_override_rate" class="mt-0.5 w-4 h-4 rounded"
                            @checked(data_get($perms, 'can_override_rate', in_array('can_override_rate', is_array($perms) ? $perms : [])))>
                        <span>
                            <span class="block text-sm font-bold text-slate-800">Override rates</span>
                            <span class="block text-[10px] text-slate-400 mt-0.5">Set a manual total different from the rate card</span>
                        </span>
                    </label>
                </div>
                <div class="mt-4 max-w-xs">
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1 mb-2">Max discount %</label>
                    <input type="number" name="max_discount_percent" min="0" max="100" step="1"
                        value="{{ old('max_discount_percent', data_get($staff->permissions ?? [], 'max_discount_percent', 10)) }}"
                        class="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition outline-none font-medium">
                </div>
            </div>
