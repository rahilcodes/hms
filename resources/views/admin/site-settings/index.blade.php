    ...
    <div x-data="{ activeTab: 'general' }" class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <form id="settings-form" action="{{ route('admin.site-settings.update') }}" method="POST"
            ...
            <div x-show="activeTab === 'payments'" class="space-y-8" x-cloak
                ...
                {{-- INDIA: GST, UPI & COMPLIANCE --}}
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-5">
                    <div>
                        <h3 class="font-bold text-lg text-slate-900">India: GST, UPI &amp; Compliance</h3>
                        <p class="text-xs text-slate-500 mt-0.5">Used on tax invoices, UPI payment QRs, review requests and the night-audit lock.</p>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Hotel GSTIN</label>
                            <input type="text" name="settings[hotel_gstin]" value="{{ $settings['hotel_gstin'] ?? '' }}"
                                placeholder="e.g. 27ABCDE1234F1Z5"
                                class="w-full p-2.5 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                            <p class="text-xs text-gray-400 mt-1">Shown on every tax invoice. Leave blank to print plain invoices.</p>
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">FSSAI License (for food bills)</label>
                            <input type="text" name="settings[hotel_fssai]" value="{{ $settings['hotel_fssai'] ?? '' }}"
                                class="w-full p-2.5 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">UPI ID (VPA)</label>
                            <input type="text" name="settings[hotel_upi_vpa]" value="{{ $settings['hotel_upi_vpa'] ?? '' }}"
                                placeholder="hotelname@okhdfcbank"
                                class="w-full p-2.5 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                            <p class="text-xs text-gray-400 mt-1">Enables Scan &amp; Pay QRs on invoices, folios and the guest portal.</p>
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">UPI Display Name</label>
                            <input type="text" name="settings[hotel_upi_name]" value="{{ $settings['hotel_upi_name'] ?? '' }}"
                                class="w-full p-2.5 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Google Review Link</label>
                            <input type="url" name="settings[google_review_link]" value="{{ $settings['google_review_link'] ?? '' }}"
                                placeholder="https://g.page/r/…/review"
                                class="w-full p-2.5 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                            <p class="text-xs text-gray-400 mt-1">Sent to guests on WhatsApp after checkout.</p>
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Owner WhatsApp (Daily Flash)</label>
                            <input type="text" name="settings[owner_whatsapp]" value="{{ $settings['owner_whatsapp'] ?? '' }}"
                                placeholder="+91XXXXXXXXXX"
                                class="w-full p-2.5 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                            <p class="text-xs text-gray-400 mt-1">Gets the occupancy / ADR / collections report after each night audit.</p>
                        </div>
                    </div>
                    <div class="flex flex-col sm:flex-row gap-4">
                        <label class="flex items-center gap-2.5 text-sm font-semibold text-gray-700">
                            <input type="checkbox" name="settings[gst_enabled]" value="1" {{ ($settings['gst_enabled'] ?? '1') === '1' ? 'checked' : '' }} class="w-4 h-4 rounded">
                            Show GST breakdown on invoices
                        </label>
                        <label class="flex items-center gap-2.5 text-sm font-semibold text-gray-700">
                            <input type="checkbox" name="settings[day_close_lock]" value="1" {{ ($settings['day_close_lock'] ?? '1') === '1' ? 'checked' : '' }} class="w-4 h-4 rounded">
                            Lock closed days after night audit
                        </label>
                    </div>
                </div>
