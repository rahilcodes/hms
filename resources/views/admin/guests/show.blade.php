            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-8 flex flex-col items-center text-center">
                ...
                <div class="flex flex-wrap justify-center gap-2 mb-4">
                    @if (($loyalty['tier'] ?? null))
                        @php
                            $tierColors = ['Platinum' => 'bg-violet-100 text-violet-700', 'Gold' => 'bg-amber-100 text-amber-700', 'Silver' => 'bg-slate-200 text-slate-600'];
                        @endphp
                        <span class="px-2.5 py-1 {{ $tierColors[$loyalty['tier']] ?? 'bg-slate-100' }} rounded-lg text-[10px] font-black uppercase tracking-widest">
                            ★ {{ $loyalty['tier'] }} — {{ $loyalty['stays'] }} stays
                        </span>
                    @endif
                    @if ($totalSpend > 5000)
                        <span
                ...
                @if(($loyalty['tier'] ?? null))
                    <div class="mb-6 px-3 py-2 rounded-xl bg-emerald-50 border border-emerald-100">
                        <p class="text-[11px] font-bold text-emerald-700">Loyalty perk: offer {{ $loyalty['discount'] }}% off direct bookings</p>
                    </div>
                @elseif(($loyalty['next_tier_at'] ?? null))
                    <p class="mb-6 text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ $loyalty['next_tier_at'] - $loyalty['stays'] }} more stays to Silver</p>
                @endif
