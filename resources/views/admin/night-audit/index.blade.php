        ...
        <div class="lg:col-span-8 space-y-6">
            ...
            <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden p-8 flex flex-col items-center text-center">
                ...
                <form action="{{ route('admin.night-audit.run') }}" method="POST" class="w-full max-w-sm">
                    ...
                    <div class="bg-slate-50 border border-slate-200 rounded-2xl p-4 mb-4 text-left">
                        <div class="flex items-center justify-between mb-2">
                            <p class="text-[10px] font-black uppercase tracking-widest text-slate-400">Cash Drawer</p>
                            <p class="text-xs font-bold text-slate-600">Expected: ₹{{ number_format($expectedCash, 2) }}</p>
                        </div>
                        <input type="number" step="0.01" min="0" name="cash_counted" placeholder="Cash counted in drawer ₹"
                            class="w-full px-3.5 py-3 rounded-xl border border-slate-200 text-sm bg-white">
                        <input type="text" name="notes" placeholder="Notes (optional)"
                            class="mt-2 w-full px-3.5 py-3 rounded-xl border border-slate-200 text-sm bg-white">
                        <p class="text-[10px] text-slate-400 mt-2">Variance vs expected cash is recorded permanently on the audit. After the audit, the day is locked — only the owner can post back-dated payments.</p>
                    </div>
