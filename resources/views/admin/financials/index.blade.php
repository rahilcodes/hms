...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white rounded-[2rem] p-8 border border-slate-200 shadow-sm relative overflow-hidden">
             <div class="absolute right-0 top-0 w-32 h-32 bg-blue-50 rounded-full blur-3xl -mr-16 -mt-16"></div>
            <div class="relative z-10">
                <p class="text-[10px] font-black uppercase tracking-[0.2em] text-blue-600 mb-4">Gross Revenue</p>
                <h3 class="text-4xl font-black tracking-tighter text-slate-900 mb-2">₹{{ number_format($stats['total_income']) }}</h3>
                <div class="flex items-center gap-2 mt-2">
                    <span class="text-xs font-bold text-slate-400">Corporate: ₹{{ number_format($stats['corporate_revenue']) }}</span>
                </div>
            </div>
        </div>
        ...
        {{-- TAXES COLLECTED --}}
        <div class="bg-white rounded-[2rem] p-8 border border-slate-200 shadow-sm relative overflow-hidden">
             <div class="absolute right-0 top-0 w-32 h-32 bg-amber-50 rounded-full blur-3xl -mr-16 -mt-16"></div>
            <div class="relative z-10">
                <p class="text-[10px] font-black uppercase tracking-[0.2em] text-amber-600 mb-4">Taxes (GST/VAT)</p>
                <h3 class="text-4xl font-black tracking-tighter text-slate-900 mb-2">₹{{ number_format($stats['taxes_collected']) }}</h3>
                <p class="text-xs text-slate-400 font-medium italic mt-2">Liabilities for remittance</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-[2rem] p-8 border border-slate-200 shadow-sm relative overflow-hidden">
            ...
            <div class="relative z-10">
                <p class="text-[10px] font-black uppercase tracking-[0.2em] text-rose-600 mb-4">Op. Expenses</p>
                <h3 class="text-4xl font-black tracking-tighter text-slate-900 mb-2">₹{{ number_format($stats['total_expense']) }}</h3>
                <p class="text-xs text-slate-400 font-medium italic mt-2">Daily operational burn</p>
            </div>
        </div>

        {{-- NET OPERATING PROFIT --}}
        <div class="bg-slate-900 rounded-[2rem] p-8 text-white relative overflow-hidden group">
            <div class="absolute -right-4 -top-4 w-40 h-40 bg-emerald-500/20 rounded-full blur-3xl group-hover:scale-110 transition-transform duration-700"></div>
            <div class="relative z-10">
                <p class="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-400 mb-4">GOP (Profit)</p>
                <h3 class="text-4xl font-black tracking-tighter mb-2 {{ ($stats['total_income'] - $stats['total_expense']) >= 0 ? 'text-white' : 'text-rose-400' }}">
                    ₹{{ number_format($stats['total_income'] - $stats['total_expense']) }}
                </h3>
                <p class="text-xs text-slate-400 font-medium italic">Revenue - Expenses</p>
            </div>
        </div>
    </div>
