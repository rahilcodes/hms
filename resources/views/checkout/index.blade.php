...
@section('content')
    <div class="bg-slate-50 min-h-screen py-10" x-data="{ 
                        ...
                        roomPrices: {
                            @foreach($selectedRooms as $item)
                                '{{ $item['cart_id'] }}': {{ $item['extra_person_price'] }},
                            @endforeach
                        },
                        ...
                        selectedUpsells: [], // Format: { service_id, cart_id, qty, ...service_data }
                        get upsellTotal() {
                        ...
                        get groupedUpsells() {
                            let groups = {};
                            this.selectedUpsells.forEach(item => {
                                if (!groups[item.id]) {
                                    groups[item.id] = { ...item, qty: 0 };
                                }
                                groups[item.id].qty += parseInt(item.qty || 1);
                            });
                        ...
                        toggleUpsell(service, cartId) {
                            const index = this.selectedUpsells.findIndex(u => u.id === service.id && u.cart_id === cartId);
                            if (index > -1) {
                                this.selectedUpsells.splice(index, 1);
                            } else {
                                this.selectedUpsells.push({ ...service, cart_id: cartId, qty: 1 });
                            }
                        },
                        isUpsellSelected(serviceId, cartId) {
                            return this.selectedUpsells.some(u => u.id === serviceId && u.cart_id === cartId);
                        },
                        getUpsellQty(serviceId, cartId) {
                            const item = this.selectedUpsells.find(u => u.id === serviceId && u.cart_id === cartId);
                            return item ? (item.qty || 1) : 0;
                        },
                        updateUpsellQty(serviceId, cartId, delta) {
                            const index = this.selectedUpsells.findIndex(u => u.id === serviceId && u.cart_id === cartId);
                            if (index > -1) {
                        ...
                        async fetchAvailabilities() {
                            this.isFetchingAvail = true;
                            try {
                                const res = await fetch(`/api/availability?check_in=${this.checkIn}&check_out=${this.checkOut}`, {
                                    headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' }
                                });
                                if (res.ok) {
                                    this.availabilities = await res.json();
                                }
                            } catch(e) {} finally { this.isFetchingAvail = false; }
                        },
                        init() {
                        ...
                        <div class="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden reveal">
                            ...
                            <div class="p-8 space-y-8">
                                ...
                                <div class="space-y-6">
                                    ...
                                    {{-- INLINE ADD ROOM SELECTOR --}}
                                    <div class="border-b border-slate-100 pb-6 mb-2">
                                        <div x-show="!showAddRoom" class="text-right">
                                            <button type="button" @click="showAddRoom = true" class="text-[10px] font-black text-blue-600 uppercase tracking-widest hover:text-blue-700 flex items-center gap-1 inline-flex group ml-auto">
                                        ...
                                        <div x-show="showAddRoom" class="p-5 bg-white border border-slate-200 rounded-[1.5rem] shadow-sm flex flex-col gap-4" x-transition x-cloak>
                                            <div class="flex justify-between items-center">
                                                <h4 class="text-[11px] font-black text-slate-800 uppercase tracking-widest">Select Room Type</h4>
                                                <button type="button" @click="showAddRoom = false" class="text-slate-400 hover:text-slate-700">
                                            ...
                                            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-start">
                                                <div class="md:col-span-3">
                                                    <select x-model="newRoomType" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500/20">
                                                        <option value="">Choose Room Type...</option>
                                                        @foreach($allRoomTypes as $type)
                                                            <option value="{{ $type->id }}"
                                                                x-text="'{{ $type->name }} (₹{{ number_format($type->base_price) }}) - ' + (isFetchingAvail ? 'Checking...' : (availabilities['{{ $type->id }}'] !== undefined ? (Math.max(0, availabilities['{{ $type->id }}'] - (cartRooms['{{ $type->id }}'] || 0))) + ' Available' : 'Checking...'))"
                                                                :disabled="availabilities['{{ $type->id }}'] !== undefined && (availabilities['{{ $type->id }}'] - (cartRooms['{{ $type->id }}'] || 0)) <= 0"
                                                            ></option>
                                                        @endforeach
                                                    </select>
                                                    <div x-show="newRoomType" class="mt-2 text-[10px] font-black uppercase tracking-wider"
                                                        :class="(availabilities[newRoomType] !== undefined && ((cartRooms[newRoomType]||0) + parseInt(newRoomQty)) > availabilities[newRoomType]) ? 'text-rose-500' : 'text-emerald-500'">
                                                        <span x-text="availabilities[newRoomType] !== undefined ? Math.max(0, availabilities[newRoomType] - (cartRooms[newRoomType]||0)) : '...'"></span> Total Available
                                                        <span x-text="'(In Cart: ' + (cartRooms[newRoomType]||0) + ')'" x-show="(cartRooms[newRoomType]||0) > 0"></span>
                                                    </div>
                                                </div>
                                                
                                                <div class="flex items-center gap-2 m-auto md:ml-auto">
                                                    <button type="button" @click="if(newRoomQty > 1) newRoomQty--" class="w-10 h-10 border border-slate-200 rounded-lg flex items-center justify-center text-slate-500 hover:bg-slate-50 bg-white font-bold">-</button>
                                                    <input type="number" x-model.number="newRoomQty" class="w-10 text-center border-none font-black text-slate-800 bg-transparent" readonly>
                                                    <button type="button" @click="newRoomQty++" class="w-10 h-10 border border-slate-200 rounded-lg flex items-center justify-center text-slate-500 hover:bg-slate-50 bg-white font-bold">+</button>
                                                </div>
                                            </div>
                                            
                                            <button type="button" 
                                                :disabled="!newRoomType || availabilities[newRoomType] === undefined || ((cartRooms[newRoomType]||0) + newRoomQty) > availabilities[newRoomType]"
                                                @click="
                                                    ...
                                                    let merged = false;
                                                    for (let id in cartRooms) {
                                                        let qt = parseInt(cartRooms[id] || 0);
                                                        if (id == newRoomType) { qt += parseInt(newRoomQty); merged = true; }
                                                        params.append(`rooms[${id}]`, qt);
                                                    }
                                                    if (!merged) {
                                                        params.append(`rooms[${newRoomType}]`, newRoomQty);
                                                    }
                                                    
                                                    window.location.href = `{{ route('checkout.show') }}?${params.toString()}`;
                                                "
                                                class="px-5 py-3 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-800 disabled:opacity-50 transition w-full mt-2 flex justify-center items-center gap-2">
                                                Add to Reservation
                                            </button>
                                        </div>
                                    </div>
                                                ...
                                                @if($item['max_extra_persons'] > 0)
                                                    <div class="flex items-center justify-between bg-white p-3 rounded-2xl border border-slate-100">
                                                        ...
                                                        <div class="flex items-center gap-3 bg-slate-50 rounded-xl p-1 border border-slate-200" x-data="{ max: {{ $item['max_extra_persons'] }} }">
                                                            ...
                                                            <input type="hidden" name="extra_persons[{{ $item['cart_id'] }}]" x-model="extraPersons['{{ $item['cart_id'] }}']">
                                                            
                                                            <button type="button" 
                                                                    @click="if(extraPersons['{{ $item['cart_id'] }}'] > 0) extraPersons['{{ $item['cart_id'] }}']--"
                                                                    ...
                                                                    :disabled="extraPersons['{{ $item['cart_id'] }}'] <= 0">
                                                            ...
                                                            <span class="text-xs font-black text-slate-900 min-w-[3rem] px-2 text-center whitespace-nowrap" x-text="extraPersons['{{ $item['cart_id'] }}'] + ' Psn'"></span>
                                                            
                                                            <button type="button" 
                                                                    @click="if(extraPersons['{{ $item['cart_id'] }}'] < max) extraPersons['{{ $item['cart_id'] }}']++"
                                                                    ...
                                                                    :disabled="extraPersons['{{ $item['cart_id'] }}'] >= max">
                                                ...
                                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                    @foreach($upsells as $service)
                                                        <div class="p-3 bg-white border border-slate-100 rounded-2xl flex flex-col justify-between transition shadow-sm relative overflow-hidden group"
                                                            :class="isUpsellSelected({{ $service->id }}, '{{ $item['cart_id'] }}') ? 'border-blue-500 ring-1 ring-blue-500 bg-blue-50' : 'hover:border-blue-300'">

                                                            <div class="flex items-center justify-between cursor-pointer"
                                                                @click="toggleUpsell({{ json_encode($service) }}, '{{ $item['cart_id'] }}')">
                                                                <div class="flex items-center gap-3">
                                                                    <div class="w-8 h-8 rounded-lg flex items-center justify-center transition"
                                                                        :class="isUpsellSelected({{ $service->id }}, '{{ $item['cart_id'] }}') ? 'bg-blue-600 text-white' : 'bg-blue-50 text-blue-600'">
                                                                ...
                                                                <div
                                                                    x-show="!isUpsellSelected({{ $service->id }}, '{{ $item['cart_id'] }}')">
                                                                    <div
                                                            ...
                                                            {{-- Quantity Selector --}}
                                                            <template
                                                                x-if="isUpsellSelected({{ $service->id }}, '{{ $item['cart_id'] }}')">
                                                                <div
                                                                    ...
                                                                    <div
                                                                        ...
                                                                        <button type="button"
                                                                            @click.stop="updateUpsellQty({{ $service->id }}, '{{ $item['cart_id'] }}', -1)"
                                                                        ...
                                                                        <span
                                                                            ...
                                                                            x-text="getUpsellQty({{ $service->id }}, '{{ $item['cart_id'] }}')"></span>
                                                                        <button type="button"
                                                                            @click.stop="updateUpsellQty({{ $service->id }}, '{{ $item['cart_id'] }}', 1)"
                ...
                <div class="lg:col-span-1">
                    <div
                        ...
                        <div class="space-y-4 mb-6">
                            ...
                            {{-- DYNAMIC UPSELLS IN SUMMARY --}}
                            <template x-for="item in groupedUpsells" :key="item.id">
                                <div
